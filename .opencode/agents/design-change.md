...
2. Ground the design in existing repository architecture and conventions, citing the Evidence IDs relied on.
...
7. Check for unnecessary abstractions, scope creep, and hidden operational or domain risks.
...
9. For every supplemental source, record Source ID, source type, path, URL, skill name, or document title, retrieved or inspected date, relevant scope, freshness requirement, and derived Evidence IDs. Summarize traceable conclusions without copying large source contents.
