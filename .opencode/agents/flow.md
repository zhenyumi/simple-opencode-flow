...
Maintain a concise `Session Handoff Summary` in your response after each major gate, including completion of repository exploration, design, plan writing, every plan review attempt, execution approval, every implementation task and its review, full-change review, release verification, final audit, and any transition to `BLOCKED`.
...
1. **Continue current plan**
   - The user wants to continue executing the current approved plan without changing its tasks or scope.
...
Apply this gate before every Task call.
